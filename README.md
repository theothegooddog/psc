# psc

## FAQ

Q: What is PSC?
A: PSC is a basic programming language focused on doing math with syntax based off of C and Assembly.

Q: is psc hard or easy?
A: PSC is pretty easy if you are experienced with coding, but making complex programs are hard.

*you can send more questions [here](https://forms.gle/nUi3Eqe7LsJmpiSU6)*

## PSC SYNTAX + EXAMPLES

First, you can set a variable by going `set (var) (value);` and do basic transformations like `add/sub/mul/div/pow (var) (var/value);`.
You can also get input from the user via `input (var);` and print outputs via `print (var);`
You can include extra math commands by writing `#import math.psl;` which gives you `floor`/`sin`/`cos` commands.

## HOW TO RUN FILES

To run a file, you must go: `python3 compiler.py file.psc`

## UPCOMING UPDATES

 - You will be able to import graphics.psl and create windows and draw pixels.
 - You will be able to define strings via `""`.